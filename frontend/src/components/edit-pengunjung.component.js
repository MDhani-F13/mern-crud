...
export default class EditPengunjung extends Component {
  constructor(props) {
    super(props);
    ...
    axios.get('http://localhost:5000/buku/')
      .then(response => {
        if (response.data.length > 0) {
          this.setState({
            buku: response.data.map(buku => buku.judul_buku),
          })
        }
      })
      .catch((error) => {
  ...
  onSubmit(e) {
    ...
    axios.post('http://localhost:5000/pengunjung/update/' + this.props.match.params.id, pengunjung)
